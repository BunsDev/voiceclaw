...
<Tabs>
  ...
  <TabItem label="Production (bundled in DMG)">
    No setup required. Open VoiceClaw and the relay starts on first launch. The Settings page shows the URL the desktop app is using.
  </TabItem>
</Tabs>
...
<Aside type="caution">
Initial release ships **arm64 only**. Intel Macs hit the "node runtime not found" branch and the desktop app continues without a bundled relay, requiring users to point at an external relay endpoint via Settings. A darwin-x64 Node download (and a `lipo`-merged universal binary) is tracked as a follow-up.
</Aside>
